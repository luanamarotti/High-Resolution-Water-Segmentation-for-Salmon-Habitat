...
All outputs are written under `BASE_DIR/results/` at runtime. The full directory (including probability maps, prediction masks, and model checkpoints) is regenerated on first run and is **not** included in this submission due to size. The small result files needed to verify the reported metrics (metric CSVs, per-fold thresholds, per-scene CSVs, scene metadata, and the stability-sweep history CSVs) **are** included, under the same structure shown below. Entries marked *(not included)* are runtime-generated and omitted from this submission.
...
│
├── unet_training_v3/
│   ├── kfold/                        # (not included — model checkpoints, regenerated on run)
...
│   ├── sweep_bs64_test/             # Stability experiment outputs (history + summary CSVs — included)
│   ├── sweep_lowwd_test/
│   ├── sweep_bce_test/
│   └── sweep_lr1e4_test/
│
├── unet_diagnostic_v3/
│   ├── prob_maps_best/              # (not included) Raw sigmoid probabilities — best single fold
│   ├── pred_maps_ensemble_majority/ # (not included) Binary predictions — majority vote ensemble
...
│
├── ndwi_chips/
│   └── masks_t0.35/                 # (not included) NDWI binary masks at t=0.35
│
├── owm_chips/                       # (not included) OWM per-chip predictions (reconstructed scenes)
├── owm_experiments/                 # (not included) OWM configuration experiment outputs
│
├── dummy_masks/
│   └── water_{pct}pct/              # (not included) Synthetic baseline masks (pct ∈ {000,020,040,050,060,080,100})
│
├── dataset_analysis/
│   ├── scene_metadata.csv           # Per-scene date, location, water fraction, split (included)
│   └── *.png                        # (not included) Temporal, spatial, seasonal, water fraction figures
│
└── dissertation_figures/            # Runtime output of generate_figures.ipynb; the final figures are
    └── fig_*.png                    # provided separately in the top-level figures/ folder of this submission
```
